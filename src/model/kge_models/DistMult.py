...
class DistMult(nn.Module):
    ...
    Implements the DistMult model from "Embedding Entities and Relations for Learning
    and Inference in Knowledge Bases" (Yang et al., 2014) for knowledge graph representation learning.

    Attributes:
        args: Configuration parameters
        kg: Knowledge graph object containing entity and relation information
        ent_embeddings: Entity embedding matrix
        rel_embeddings: Relation embedding matrix (diagonal)
    """

    def __init__(self, args, kg) -> None:
        """
        Initialize DistMult model with knowledge graph and configuration parameters
    ...
    def score_fun(self, h: torch.Tensor, r: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        Calculate DistMult score using trilinear product

        Higher scores indicate higher plausibility
        """
        h = self.ent_norm(h)
        r = self.rel_norm(r)
        t = self.ent_norm(t)
    ...
    def prepare_indices(self, h, r=None, t=None):
        ...
        # Case 2: Separate head, relation, tail
            ...
            raise ValueError("Invalid input format for DistMult model")
    ...
    def predict(self, head, relation):
        ...
        # DistMult scoring: score(h,r,t) = <h, r, t> = sum(h * r * t)
        # For prediction, we compute h * r and then dot product with all entities
        hr = h * r

        # Score against all possible tail entities
        score = torch.mm(hr, t_all.transpose(0, 1))
    ...
    def forward(self, head, relation=None):
        ...
        # Split input tensor
        h, r = self.prepare_indices(head, relation)

        # Get embeddings
        ent_embeddings, rel_embeddings = self.embedding()

        # Lookup embeddings
        h = torch.index_select(ent_embeddings, 0, h)
        r = torch.index_select(rel_embeddings, 0, r)
